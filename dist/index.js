...
const latestPath = path_1.default.join(dataDir, "latest.json");
const historyPath = path_1.default.join(dataDir, "history.json");
const state = {
    latest: null,
    history: []
};
...
function parseScheduleDate(rawText) {
...
function isScheduleUpdate(rawText) {
...
function formatScheduleLog(item) {
    if (!item.schedule || item.schedule.length === 0)
        return "";
    const lines = item.schedule.map((entry) => `${entry.queue}: ${entry.ranges.join(", ")}`);
    return lines.join("\n");
}
async function notifyTelegram(newItems) {
    if (!BOT_TOKEN || !CHAT_ID || newItems.length === 0)
        return;
...
async function bootstrap() {
    await ensureDataDir();
    state.latest = await readJsonFile(latestPath, null);
    state.history = await readJsonFile(historyPath, []);
    await pollChannel();
